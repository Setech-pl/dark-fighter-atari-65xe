...
6564-bajtowy strumień LZ-10/5, etapowane pod `$8100-$9AA3` i odtwarzane pod
`$21C1-$3FFF`. Przed tym odtworzeniem bootstrap rozwija 5632-bajtowy packed
broadside do `$5E10-$780C` i zachowuje pozostałe źródła: 1731 B
starfield/music pod `$7810-$7ED2` oraz 226 B A2 pod `$7F10-$7FF1`. Po
skopiowaniu tych źródeł 1493 B ENTITY_CODE jest etapowane w odzyskiwanym
zakresie loadera `$5200-$57D4`. A2 jest następnie kopiowany do
`$9000-$90E1`, a ENTITY_CODE rozwijany do `$9100-$97CF`. Dopiero
...
relocated broadside/frontend/pause/weapon data occupy 6653/6656 B at `$5E10-$780C`.
...
of 6564 B for the resident suffix, 5632 B for broadside, 1731 B for starfield
and 1493 B for ENTITY_CODE; the product-preserving A2 kernel occupies 226/256 B
at `$9000`. Sam bounded
...
33 036 cykli DMA-on wall, czyli 2 532 cykle fizycznego headroom. Jest to +80
względem baseline Spread 32 956; target/hard 33 212/33 340 i minimum 2 200
przechodzą z zapasem. Pełna macierz baseline, targeted, cadence, effects,
weapon-pickup, 120-sekundowa XEX/ATR, cold `$A5/$5A` oraz restart obserwuje
naprzemienny cykl kapsuł i trzystrzałowe salwy bez opuszczonych synchronizacji,
dodatkowych granic VBI ani deadline overruns. Szczegóły
...
four physical slots. Slot 0 is fixed debris, slot 1 is fixed weapon-pickup state,
slot 2 provides non-rendered timed-booster storage, slot 3 remains reserved,
and the interactive release active limit is still two. Effects
...
`$65`. Executable cold-boot tests fill RAM with `$A5` and `$5A`, run the packed
XEX and ATR bootstrap paths, compare all 256 bytes and check guard bytes.

`ENTITY_CODE` runs at `$9100-$97CF` and is stored as a 1493-byte LZ-10/5 boot
stream. Its 1744 B contain 1523 B entity/effect/booster and cold-initialization code
...
`$A000-$BFFF`. The resident suffix is cold-staged under `$8100-$99A3`; packed
ENTITY_CODE is staged at `$5200-$57D4` only after the starfield/A2 sources have
been copied and is unpacked before the loader bitmap claims that destination.
`$8100-$8FFF` and `$97D0-$9FFF` remain reserved after startup.
...
### Rapid Fire and Spread Shot weapon pickups
...
resets the counter and enters PENDING. Each successful creation copies the next
type from reserved slot-2 `ENTITY_TYPE`, then toggles it Rapid→Spread→Rapid;
the New Game page clear restores Rapid as the first drop. Other damage sources
never touch the counter, and any non-idle slot-1 state blocks a second drop
cycle. The interactive active limit stays two and the effects pool is unchanged.
...
Sector transition clears only a pending/visible capsule; either collected timer
survives a live sector change. Life loss, Game Over and new game clear it;
pause executes no timer update.

The fixed renderer reads the spawn-owned render ID but no animation phase.
Rapid uses glyphs/codes 120–123: steel `COLPF1=$84`, yellow `COLPF2=$1E` and
tall black R/F cut-outs, without selector `01`/white. Spread uses inverse codes
252–255 for glyphs 124–127: the existing D7 routing makes its dense casing
red `COLPF3=$46`, while a large black three-way fan is cut through the interior.
Both 2×2 capsules are static and continuously visible. No DLI, PMG or global
palette write is introduced; all gameplay glyphs 0–127 are now occupied.
...
Collection removes slot-1 mask/backing once and installs a non-rendered
16-bit controller in reserved slot 2: `ENTITY_STATE+2` is state 3 RAPID or
state 4 SPREAD, while `ENTITY_TIMER+2` is low and
`ENTITY_MOVE_ACCUMULATOR+2` high. It loads 500 and decrements only on active
gameplay frames. Slot 1 immediately returns to capsule duty, so a later pickup
can coexist with the active controller and naturally replace the other type or
refresh the same type. `update_viper_weapon` chooses interval 2 instead of 3
only when the controller is RAPID; burst size 10,
post-burst cooldown, projectile pool, geometry, damage and rejection semantics
are unchanged. `ENTITY_OWNER+2` is the 50-frame HUD subcounter and
`ENTITY_HP+2` holds the current seconds digit code during either booster. HUD
cells 32–35 show `RF10..RF01` or `SP10..SP01`. A Viper
...
For SPREAD, one accepted emission first proves that at least three of the ten
physical Viper slots are free, then allocates left/centre/right atomically.
Their active bytes are `$C1/$01/$A1`: bits 6/5 encode the ±2 HPOS step and D7
routes only the side shots to existing red `COLPF3`; the centre remains yellow.
All three move upward at the normal 6 scanlines/frame, share the established
swept Raider/debris arbitration and are freed before their top/side pointers
can escape the display. Two free slots therefore reject the complete volley
without a partial projectile. Reverse erase remains the existing descending
ten-slot pass; no new projectile, entity or effect storage is allocated.
...
| 2. Boot payload i sektory startowe | Payload is exactly 16,384 B in 128 full sectors. It contains a 449 B raw bootstrap, 6564 B packed resident suffix, 5632 B broadside, 1731 B starfield, 226 B A2 kernel, 1493 B ENTITY_CODE, a 285 B source-owned zero reserve at `$5EDF-$5FFB`, and the `DFB1` trailer at `$5FFC-$5FFF`. Formatter-added padding is forbidden. XEX is 16,396 B including headers/RUNAD. Load address `$2000`, entry `$201E`, loader field and media format are unchanged. | Raportować osobno rozmiar payloadu, liczbę sektorów, source-owned reserve/trailer, XEX headers i granice czasowe relokacji. Nie podnosić ani nie rozluźniać inwariantu 128 sektorów bez owner approval. |
| 3. Resident gameplay RAM | After cold unpack, CODE/RODATA use 8190/8192 B at `$2000-$3FFD`; the hull maps use 576 B at `$4C00-$4E3F`, persistent state ends at `$4EE9`, HUD charset uses `$5000-$53FF`, projectile/far-star state uses `$5400-$5529`, starfield/shared/music runtime uses 2191/2278 B at `$552A-$5DB8`, and broadside/frontend/pause runtime uses 6653/6656 B at `$5E10-$780C`. A2 LMS/ring state uses 203 B at `$7F10-$7FDA`; ENTITY_STATE uses `$8000-$80FF`, A2 kernel `$9000-$90E1`, and ENTITY_CODE 1744/3840 B at `$9100-$97CF`. | `$97D0-$9FFF` stays reserved after cold staging is consumed. `$A000-$BFFF` remains excluded until PORTB/BASIC-ROM mapping is proved on XEX, cold ATR and hardware. |
| 4. Pamięć przejściowa loadera | Before the loader, bootstrap stages the packed resident suffix at `$8100-$9AA3`, starfield at `$7810-$7ED2`, A2 at `$7F10-$7FF1`, and—after those sources have been consumed—ENTITY_CODE at `$5200-$57D4`. It expands broadside before restoring `$21C1-$3FFF`, then expands ENTITY_CODE, initializes BSS/effects, copies A2, displays the loader and finally expands starfield. The raw loader bitmap still occupies `$4010-$5E0F`; its source is consumed before the display-list stream expands to fixed `$3800`. | Wszystkie zakresy w tym wierszu są cold-start staging, nie nowym stanem ani rezerwacją gameplay. `$A000-$BFFF` is never used. Koszt raportować jako setup/transient. |
| 5. Pamięć odzyskiwalna po przejściu | Only actual single-line PMG DMA pages `$3B00-$3FFF` are cleared. `$3800-$3AFF` contains non-DMA resident/loader RODATA and is preserved. After unpacking, `$7810-$7BCF` is reused only while PAUSED and `$7F10-$7FDA` holds A2 state. Frontend uses `$4800-$4BFF`, maps/state `$4C00-$4EE9`, HUD charset `$5000-$53FF`, projectile/far state `$5400-$5529`, starfield runtime `$552A-$5DB8`, broadside runtime `$5E10-$780C`, entity state `$8000-$80FF`, A2 `$9000-$90E1`, and entity code `$9100-$97CF`. | Reuse wymaga jawnej rezerwacji, testu przejścia i aktualizacji memory map. Nie ma dynamicznego allocatora. |
...
| 7. Display memory i charset | Wspólny ekran `$4000-$43FF`; gameplay charset `$4400-$47FF`; frontend charset `$4800-$4BFF`; HUD charset `$5000-$53FF`. Starfield occupies indices 1–6, Viper fire starts at 11, hull/effects at 59, Raider fire at 90, neutral debris uses 110–117, fragment phases 118–119, Rapid uses 120–123 and Spread uses 124–127. All 128 gameplay glyphs are occupied. Main menu uses 820 B, sub-screens/gameplay 960 B. Two 75 B LMS lists preserve HUD `$4000`, divider `$4028` and rotate exactly 22 rows `$4050-$43BF`. | Kolejne glify wymagają zwolnienia istniejącego indeksu albo jawnej zmiany architektury; każdy runtime charset nadal ma dokładnie 1024 B. |
...
| 9. Koszt widocznej ramki | The artifact-bound matrix includes the 3200-frame hunt replay, 6000 active XEX and 6000 active ATR integrity frames, cold `$A5/$5A`, all difficulties and restart coverage. It observes the alternating created-capsule cycle, Spread volleys, a capsule coexisting with an active booster, and a legal worst frame with one active capsule plus eight Viper projectiles. Final worst wall is 33,036 cycles, 80 above the 32,956 Spread baseline, with 2,532 physical headroom and zero missed synchronisations, extra VBI boundaries or deadline overruns. | Spread target/hard limits are 33,212/33,340 and minimum headroom is 2,200; current result retains 176 cycles to target and 304 to hard. |
| 10. Jednorazowy setup/transition | `start` stages and expands the 6564 B resident suffix into 7743 B, expands 5632 B into 6653 B broadside and 1493 B into 1744 B ENTITY_CODE, clears the same 256 B BSS, preserves 1731 B at `$7810`, expands it into 2191 B starfield runtime and copies 226 B A2 kernel. The contiguous 202-byte projectile/burst/explosion state is unchanged and cleared by one bounded reset loop; this cold/new-game work is outside the visible-frame path. Pause enter/exit still copies 960 B only with gameplay DMA disabled. |

The raw linker image ends at `$3FFD`, leaving two bytes in MAIN. Packaging
preserves `$2000-$21C0` verbatim and replaces the remaining
resident suffix with its LZ-10/5 stream, followed by the relocated packed
runtime sources, 285 B of explicit reserve and `DFB1`. These source ranges are
