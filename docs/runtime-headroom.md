...
padding. Its source-owned layout ends with 1,097 zero reserve bytes at
`$5BB3-$5FFB` and the four-byte `DFB1` trailer at `$5FFC-$5FFF`; build and
validation reject a reserve below 1,024 B, any other payload length, sector
...
ENTITY_STATE 256/256 B and ENTITY_CODE 849/3,840 B. Cold staging uses
`$8100-$9D80` only before gameplay ownership begins; the separate ENTITY_CODE
boot stream is 772 B.
...
ENTITY_CODE at `$9100-$9450`; after cold staging, `$8100-$8FFF` and
`$9451-$9FFF` remain reserved and untouched. `$A000-$BFFF` is excluded.
The accepted gameplay/effect bytes through `$93D4` remain unchanged; the
additional 124 B are relocated cold initialization. Debris uses exactly
